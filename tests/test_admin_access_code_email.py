"""
Emailing an assigned access code to its recipient — app/routers/admin_router.py.

An ASSIGNED code (assigned_to_email set) is reserved for one specific
person; this covers actually getting the code INTO their inbox, either
automatically on creation (send_email=True, the default) or on-demand via
the resend endpoint — rather than requiring the admin to copy/paste it
into some other channel by hand.
"""
import asyncio
...
def test_creating_an_assigned_code_emails_it_by_default(fake_send):
    ...
    body = CreateAccessCodeRequest(
        ...
        assigned_to_email="Partner@Example.com", label="Africa SME Assembly",
    )
    ...
    assert fake_send[0]["to_email"] == "partner@example.com"  # normalized lowercase
...
def test_send_email_false_skips_the_email(fake_send):
    ...
    body = CreateAccessCodeRequest(
        code="ASA26", plan_tier_id="starter", duration_days=60,
        assigned_to_email="partner@example.com", send_email=False,
    )
    result = _run(create_access_code(body, admin_user=_admin(), db=db))
    _flush()
...
def test_resend_endpoint_emails_the_currently_assigned_person(fake_send):
    ...
    result = _run(send_access_code_email("asa26", admin_user=_admin(), db=db))
    _flush()

    assert result == {"sent": True, "to": "partner@example.com"}
    assert len(fake_send) == 1
    assert fake_send[0]["to_email"] == "partner@example.com"
