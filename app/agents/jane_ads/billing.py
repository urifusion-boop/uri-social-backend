...
async def reconcile_ad_spend_charges(db) -> dict:
    ...
    for r in records:
        ...
        billed = float(r.get("spend_billed_ngn", 0.0))
        ...
        charge = round(min(owed, balance), 2)               # take what the wallet can cover
        ran_dry = charge < owed

        if charge > 0:
            # The portion of Meta spend this charge actually recoups (charge ÷ markup)
            # — advance the mark by exactly that, so any shortfall stays billable.
            spend_covered = round(charge / markup, 2)
            try:
                await wallet.charge_ad_spend(
                    business_id, charge, campaign_id=campaign_id, meta_spend_ngn=spend_covered,
                )
            except InsufficientFundsError:
                # Balance moved under us (shouldn't, single-threaded sweep) — treat as dry.
                ran_dry = True
                spend_covered = 0.0
            if spend_covered > 0:
                await db[COLLECTION].update_one(
                    {"campaign_id": campaign_id},
                    {"$set": {"spend_billed_ngn": round(billed + spend_covered, 2),
