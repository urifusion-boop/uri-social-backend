...
import httpx
from fastapi import (
    APIRouter, Body, Depends, File, Header as _FastapiHeader, HTTPException, Query, Request, UploadFile,
)
...
# TEMPORARY (re-added) — verify the scrim / solution-copy fix on a real
# render before asking the user to regenerate again. Delete after.
@router.post("/admin/debug-vsg01-generate")
async def debug_vsg01_generate(
    body: dict = Body(...),
    x_bootstrap_secret: str = _FastapiHeader(...),
    db: AsyncIOMotorDatabase = Depends(get_db_dependency),
) -> dict:
    ...
    return {
        "flag": settings.JANE_ADS_VSG01_ENABLED,
