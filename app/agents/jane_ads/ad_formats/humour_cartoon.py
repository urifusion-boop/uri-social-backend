...
def _illustration_prompt(situation: str, nigerian_setting: str) -> str:
    return (
        ...
        "clean confident linework with varied line weight, bold saturated flat "
        "colours, exaggerated expressive poses and faces that sell the joke at "
        "a glance, dynamic composition with a clear foreground/background "
        "read, one single clear visual punchline, polished professional comic "
        "illustration style. Fill the background with specific, recognisable "
        "environmental detail from the setting itself, not a flat empty "
        "colour field. Any "
