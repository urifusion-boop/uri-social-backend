...
def logo_badge_layers(
    ...
    """A small bottom-right logo mark — the brand's real logo image placed
    directly, no backing shape — used by every format whose own
    build_document() doesn't already reserve dedicated header space for a
    brand mark (Receipt and Review Card do their own top-of-canvas
    placement; this is the shared fallback for the rest). Returns ([],
    z_start) unchanged when there's no real logo to place — never fabricates
    one, matching every other brand_logo_url caller's own contract. Purely
    additive (drawn last, on top of existing content) so it never risks the
    careful per-format wrap/overflow math each layout already has tuned.

    Previously drew a white rounded-rect backing behind the logo "for
    legibility over any content underneath" — live-reported as an unwanted
    white box (the brand's own logo asset is already a real transparent PNG,
    same as everywhere else in the app that composites it; this was the only
    place that boxed it in rather than placing it directly).
    """
    if not brand_logo_url:
        return [], z_start
    badge_w, badge_h, margin = 140, 56, 24
    x = canvas_width - badge_w - margin
    y = canvas_height - badge_h - margin
    z = z_start
    layers = [
        {
            "type": "brand_asset", "z_index": z + 1,
