...
def _problem_prompt(problem_situation: str, nigerian_setting: str, seasonal_context: Optional[str] = None) -> str:
    ...
    return (
        ...
        f"{REPRESENTATION_BLOCK}. Frame the shot so at least one person's face "
        "and upper body are clearly visible — never crop out heads or faces; a "
        "shot of legs or backs alone does not communicate the problem. Show one "
        "clear human situation that communicates the problem within "
        "approximately one second, with the relevant object or context "
        "visibly in frame. Use muted, slightly desaturated natural colours "
        "with overcast or shaded daylight. The image should feel observed "
        f"rather than staged: realistic, imperfect, unpolished and documentary.{(' ' + seasonal) if seasonal else ''}"
    )
...
def _solution_prompt(
    ...
    return (
        ...
        f"{REPRESENTATION_BLOCK}. This must depict the SAME physical location "
        "and scene as the paired problem photo — not a different place — at a "
        "visibly more organised, successful or satisfying moment. Use the same "
        "camera distance and framing (close-up stays close-up, wide stays "
        "wide) so the two read as before/after of one continuous story. Use "
        "brighter natural daylight and warmer but realistic tones. Do not make "
        "the result look exaggerated, luxurious or impossibly perfect. The "
        "contrast should come primarily from the situation itself, not "
        f"excessive colour grading.{(' ' + seasonal) if seasonal else ''}"
    )
